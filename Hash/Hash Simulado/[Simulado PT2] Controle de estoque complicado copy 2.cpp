...
int main()
{
	...
	for (int j = 0; j < operacoes; j++)
	{
		...
		if (op == 'I')
		{
			...
			pos_inserido = buscar(tabela_hash, m, inserido.codigo);
			if (pos_inserido == -1)
			{
			...
			{
				tabela_hash[pos_inserido].quantidade += inserido.quantidade;
				tabela_hash[pos_inserido].preco += inserido.preco;
				tabela_hash[pos_inserido].preco /= 2;
			}
		}
		else if (op == 'V')
		{
